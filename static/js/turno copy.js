...
async function loadCalendar() {
  const res = await fetch(`/week-availability?week_offset=${weekOffset}&t=${Date.now()}`);
  ...
  data.days.forEach(day => {
    ...
    day.slots.forEach(slot => {
      ...
      slotDiv.className = "slot " + (slot.available ? "available" : "busy");
...
ddocument.getElementById("confirmar-btn").addEventListener("click", async () => {
  ...
  const startTime = `${selectedDate}T${selectedTime}:00Z`; // ← mejor ISO
  ...
  try {
    ...
    if (res.ok) {
      ...
      btnConfirmar.innerText = "Confirmar turno";
    }
